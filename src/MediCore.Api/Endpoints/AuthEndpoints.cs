...
public static class AuthEndpoints
{
    public static IEndpointRouteBuilder MapAuthEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/users", RegisterAsync)
    ...
    private static async Task<IResult> BootstrapAdminAsync(
        ...
        ToResult(await authService.BootstrapAdminAsync(request, cancellationToken));

    private static async Task<IResult> LoginAsync(
        LoginRequest request,
        IAuthService authService,
        CancellationToken cancellationToken) =>
        ToResult(await authService.LoginAsync(request, cancellationToken));

    private static async Task<IResult> RefreshAsync(
        RefreshTokenRequest request,
        IAuthService authService,
        CancellationToken cancellationToken) =>
        ToResult(await authService.RefreshAsync(request, cancellationToken));

    private static async Task<IResult> LogoutAsync(
        LogoutRequest request,
        IAuthService authService,
        CancellationToken cancellationToken) =>
        ToResult(await authService.LogoutAsync(request, cancellationToken));

    private static async Task<IResult> RegisterAsync(
        RegisterUserRequest request,
        IAuthService authService,
        CancellationToken cancellationToken) =>
        ToResult(await authService.RegisterAsync(request, cancellationToken));
