...
if (builder.Configuration.GetValue<bool>("Database:InitializeOnStartup")) await app.Services.InitializeMediCoreDatabaseAsync();

app.UseExceptionHandler();
app.UseCors("MediCoreWeb");
app.UseAuthentication();
app.UseAuthorization();
app.MapGet("/api", () => Results.Ok(new { name = "MediCore.Api", slogan = "La gestión médica en un solo lugar.", version = "0.7.0-phase6" }));
...
static Task WriteHealthResponse(HttpContext context, HealthReport report)
{
    ...
    return context.Response.WriteAsync(JsonSerializer.Serialize(new
    {
        ...
        checks = report.Entries.Select(entry => new { name = entry.Key, status = entry.Value.Status.ToString(), description = entry.Value.Description, durationMs = entry.Value.Duration.TotalMilliseconds })
    }));
}
